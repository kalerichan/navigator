...
# ================== ПРОВЕРКА ПОДПИСКИ (С ЛОГАМИ) ==================
async def is_subscribed(bot, user_id):
    try:
        chat_member = await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=user_id)
        status = chat_member.status
        logging.info(f"Статус пользователя {user_id} в канале {CHANNEL_ID}: {status}")
        # Если статус member, administrator, creator – подписан
        if status in ("member", "administrator", "creator"):
            return True
        else:
            logging.warning(f"Пользователь {user_id} не подписан (статус: {status})")
            return False
    except Exception as e:
        logging.error(f"Ошибка проверки подписки для {user_id}: {e}")
        # Если ошибка, предположим, что подписки нет (или бот не админ)
        return False
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Далее проверяем подписку для остальных действий
    if not await is_subscribed(context.bot, user_id):
...
# ===== Тексты утренних заданий (сокращённо, полные в предыдущем коде) =====
MORNING_TEXTS = {
    (1,1): "☀️ День 1. Мои точки опоры...",
    # ... полные тексты (можно взять из предыдущего кода)
}
