...
async def send_morning_task(update: Update, context: ContextTypes.DEFAULT_TYPE, track: int, day: int):
    ...
    if not await is_subscribed(context.bot, user_id):
        ...
        await context.bot.send_message(
            chat_id=user_id,
            text="❌ Ой, кажется, ты отписалась от канала. Чтобы получать задания челленджа, подпишись снова, пожалуйста 💔",
            reply_markup=reply_markup
        )
        return
...
async def send_evening_audio(chat_id, audio_path, track, day):
    try:
        ...
        if not await is_subscribed(bot, chat_id):
            ...
            await bot.send_message(
                chat_id=chat_id,
                text="❌ Ой, кажется, ты отписалась от канала. Чтобы получить вечернее голосовое сообщение, подпишись снова 💔",
                reply_markup=reply_markup
            )
            return
...
async def send_morning_task_by_chat(chat_id, track, day):
    ...
    if not await is_subscribed(bot, chat_id):
        ...
        await bot.send_message(
            chat_id=chat_id,
            text="❌ Ой, кажется, ты отписалась от канала. Чтобы получить задания, подпишись снова 💔",
            reply_markup=reply_markup
        )
        return
